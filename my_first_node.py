...
class WalkForward(Node):
    ...
    def walk_forward_with_world_frame_goal(self, ROBOT_T_GOAL) -> None:
    ...
    def sit(self) -> bool:
        # sit the robot down
        self.get_logger().info("Making the Robot Sit")
        result = self._robot.command("sit")
        if not result.success:
            self.get_logger().error("Robot did not sit: " + result.message)
            return False
...
def main() -> int:
    ...
    goto.initialize_robot() # power on robot and make stand

    # goto.walk_forward_with_world_frame_goal(SE2Pose(1.0, 0.0, 0.0)) # walk forward

    # move forward/backward, left/right, turn
    goto.walk_forward_with_world_frame_goal(SE2Pose(0.3, 0.0, 0.0)) # forward
    goto.walk_forward_with_world_frame_goal(SE2Pose(0.0, 0.3, 0.0)) # left
    goto.walk_forward_with_world_frame_goal(SE2Pose(-0.3, 0.0, 0.0)) # backward
    goto.walk_forward_with_world_frame_goal(SE2Pose(0.0, -0.3, 0.0)) # right

    # full spin
    goto.walk_forward_with_world_frame_goal(SE2Pose(0.0, 0.0, -3)) # turn left
    goto.walk_forward_with_world_frame_goal(SE2Pose(0.0, 0.0, -3)) # turn left


    # sleep(3) # wait 3 seconds
    goto.sit() # make the robot sit
